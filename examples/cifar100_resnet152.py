...
def run(seed: int | None) -> tuple[dict[str, float], Metrics, int]:
    ...
    bitnet = resnet152(BitLinear, BitConv2d, pretrained=True, num_classes=num_classes)
    floatnet = resnet152(nn.Linear, nn.Conv2d, pretrained=True, num_classes=num_classes)
